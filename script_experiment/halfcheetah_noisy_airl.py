...
def main():
    ...
    set_seed(0)
    env = TfEnv(CustomGymEnv('HalfCheetah-v3', record_video=False, record_log=False))

    with open("./demos/suboptimal_demos/halfcheetah/dataset.pkl", "rb") as f:
    ...
    algo = IRLTRPO(
        ...
        gae_lambda=0.95,
        step_size=0.01,  # max_kl for TRPO
        optimizer_args=dict(reg_coeff=0.1, cg_iters=15)  # reg_coeff=1e-5
    )
